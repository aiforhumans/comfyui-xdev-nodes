...
# Setup InsightFace model paths for ComfyUI model management
debug_print("Setting up model paths...")
try:
    import folder_paths
    import os
    
    # Add InsightFace model directories to ComfyUI model management
    ...
    # Add face swap models directory  
    ...
    debug_print(f"✅ Model paths registered: {insightface_models_dir}, {faceswap_models_dir}")
except Exception as e:
    debug_print(f"⚠️ Model path setup failed (ComfyUI not available): {e}")
...
debug_print("Loading advanced sampling...")
from .nodes.sampling_advanced import AdvancedKSampler, VariantSelector
debug_print("✅ Advanced sampling loaded")

debug_print("Loading InsightFace model loaders...")
try:
    from .nodes.insightface_loaders import XDEV_InsightFaceModelLoader, XDEV_InsightFaceSwapperLoader, XDEV_InsightFaceProcessor
    debug_print("✅ InsightFace loaders loaded")
except Exception as e:
    debug_print(f"⚠️ InsightFace loaders failed to load: {e}")
    # Create placeholder classes for graceful degradation
    class XDEV_InsightFaceModelLoader:
        pass
    class XDEV_InsightFaceSwapperLoader:
        pass
    class XDEV_InsightFaceProcessor:
        pass

debug_print("Loading InsightFace face swap...")
try:
    from .nodes.insightface_faceswap import XDEV_InsightFaceFaceSwap
    debug_print("✅ InsightFace face swap loaded")
except Exception as e:
    debug_print(f"⚠️ InsightFace face swap failed to load: {e}")
    # Create placeholder class
    class XDEV_InsightFaceFaceSwap:
        pass

debug_print("Loading professional face swap nodes...")
try:
    from .nodes.faceswap_professional import XDEV_FaceExtractEmbed, XDEV_FaceSwapApply
    debug_print("✅ Professional face swap nodes loaded")
except Exception as e:
    debug_print(f"⚠️ Professional face swap nodes failed to load: {e}")
    # Create placeholder classes for graceful degradation
    class XDEV_FaceExtractEmbed:
        pass
    class XDEV_FaceSwapApply:
        pass

debug_print("Loading face swap nodes...")
try:
    from .nodes.face_swap import XDEV_AdvancedFaceSwap, XDEV_FaceSwapBatch, XDEV_FaceQualityAnalyzer
    debug_print("✅ Face swap nodes loaded")
except Exception as e:
    debug_print(f"⚠️ Face swap nodes failed to load: {e}")
    # Create placeholder classes for graceful degradation
    class XDEV_AdvancedFaceSwap:
        pass
    class XDEV_FaceSwapBatch:
        pass 
    class XDEV_FaceQualityAnalyzer:
        pass
...
NODE_CLASS_MAPPINGS = {
    ...
    # Phase 8 Advanced Sampling
    "XDEV_AdvancedKSampler": AdvancedKSampler,
    "XDEV_VariantSelector": VariantSelector,
    # Phase 9 InsightFace Integration
    ...
    # Phase 10 Professional Face Swap (InsightFace + InSwapper)
    "XDEV_FaceExtractEmbed": XDEV_FaceExtractEmbed,
    "XDEV_FaceSwapApply": XDEV_FaceSwapApply,
    # Phase 11 Advanced Face Swap
    "XDEV_AdvancedFaceSwap": XDEV_AdvancedFaceSwap,
    "XDEV_FaceSwapBatch": XDEV_FaceSwapBatch,
    "XDEV_FaceQualityAnalyzer": XDEV_FaceQualityAnalyzer,
}
...
NODE_DISPLAY_NAME_MAPPINGS = {
    ...
    # Phase 8 Advanced Sampling
    "XDEV_AdvancedKSampler": "Advanced KSampler (XDev)",
    "XDEV_VariantSelector": "Variant Selector (XDev)",
    # Phase 9 InsightFace Integration
