    ...
    def build_prompt(self, template, subject="", style="", details="", 
        ...
        try:
            ...
            try:
                ...
                logger.warning("Invalid additional_vars JSON, ignoring")
...
class XDEV_TextToImagePromptBridge(ValidationMixin):
    ...
    def convert_to_image_prompt(self, llm_response, extraction_mode="clean_and_format",
        ...
        try:
            ...
            # Build negative prompt from negative terms
            negative_prompt = negative_terms if negative_terms.strip() else ""
            
            # Processing info
            original_length = len(llm_response)
            final_length = len(prompt)
            processing_info = f"Mode: {extraction_mode}, Original: {original_length} chars, Final: {final_length} chars"
            
            return (prompt, negative_prompt, processing_info)
            
        except Exception as e:
            error_msg = f"Prompt Bridge Error: {str(e)}"
            logger.error(error_msg)
            return (llm_response[:max_length], negative_terms, error_msg)

    def _extract_description(self, text: str) -> str:
        """Extract descriptive content, remove conversational fluff"""
        # Remove common conversational phrases
        conversational_phrases = [
            "I'll create", "Here's", "Let me", "I can see", "This image shows",
            "The image depicts", "I would describe", "This appears to be"
        ]
        ...
        # Remove questions and first-person references
        sentences = cleaned.split('.')
        descriptive_sentences = []
        
        for sentence in sentences:
            sentence = sentence.strip()
            if sentence and not sentence.endswith('?') and 'I ' not in sentence:
                descriptive_sentences.append(sentence)
        
        return '. '.join(descriptive_sentences).strip()
    ...
    def _extract_from_json(self, text: str) -> str:
        ...
        try:
            ...
            # Common JSON keys for prompts
            prompt_keys = ["prompt", "description", "image_prompt", "scene", "content"]
    ...
    def _clean_and_format(self, text: str) -> str:
        ...
        # Remove quotation marks and other formatting
        cleaned = cleaned.replace('"', '').replace("'", "")
        
        # Extract core descriptive elements
        cleaned = self._extract_description(cleaned)
